...
public class ApplicationMasterService extends AbstractService implements
  ...
  public AllocateResponse allocate(AllocateRequest request)
    ...
    synchronized (lock) {
      ...
      //LOG.info("Received allocate request from appmaster");
      LOG.info("allocate on AM request  appAttemptId : "+appAttemptId);
      Allocation allocation =
          this.rScheduler.allocate(appAttemptId, ask, release, 
              blacklistAdditions, blacklistRemovals);
      LOG.info("allocation is done appAttemptid: "+appAttemptId +" container size :  " +allocation.getContainers().size()+ " resource memory: "+allocation.getResourceLimit().getMemory());
